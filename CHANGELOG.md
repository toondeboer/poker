...
- **Payouts (Pro).** Set a buy-in and the app works out what each place wins, so the split is agreed
  ...
  - **Rebuys and add-ons** are counted too. A rebuy is another buy-in — it grows the prize pool and
    ...
    not a bounty, so all of it goes to the prize pool and it can carry its own price. Crucially
    neither changes **how many places get paid**: that follows the number of *players*, so thirteen
    players with five rebuys is still thirteen people to pay out to, with a bigger pool between
    them.
