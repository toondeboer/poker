...
- **Where a dependency goes: the workspace that imports it, and nowhere else.**
  ...
  - **`packages/core`** — devDependencies only, because it has no runtime dependencies at all and
    should stay that way. If it ever needs one, it goes in `dependencies`; anything the _host_ app
...
- **Don't move the sheets to `presentation: "formSheet"` — it was tried and reverted.**
  ...
  - **Retested on `react-native-screens` 4.26.2 (SDK 56, RN 0.85.3, iOS 26.5 simulator) — still
    broken. Don't spend the day on it a third time.** The flag _did_ change: in 4.25.2
    ...
    - RNS's own source says of the no-bottom-constraint style: _"It was tested reliably only on
      Android."_ Take that at face value.
...
- **Never commit directly to `release/<version>`, even for a docs-only change** (e.g. a
  ...
  _cutting_ the release itself (step-by-step below) — those go directly on the release branch.
- **What targets the release branch, and what targets `main`** — the test is _"does this change the
  mobile binary?"_, and it only applies **while a release branch is open**:
  ...
  - **The one exception: web copy that describes an unreleased app targets the release branch.**
    The rule above exists so web _fixes_ aren't held hostage to App Store review. It does not
  ...
  - **Dependabot targets `main`, and that's correct** — don't set `target-branch` in
    ...
    redirects _version_ updates — **security updates are raised against the default branch
  ...
  - **The real filter for a mobile dependency isn't semver, it's the SDK.** Run
    `npx expo install --check` in `apps/mobile`: it reports what the _installed_ Expo SDK expects,
...
- **Merging a stack of dependent PRs: never `--delete-branch` one that another PR is based on, and
  ...
  - After a squash merge the parent's content is on the release branch as a **different commit**, so
    ...
    _before the merge_: if you already rebased and force-pushed the parent, the tip you need is the
...
- **Open the `release/<version>` → `main` PR as soon as the first change lands on the branch, and
  leave it open** (`gh pr create --base main --head release/<version>`). Not _immediately_ after
...
- **Cutting the release**, once everything intended for it is merged into `release/<version>`:
  1. Bump native version files for whichever platform(s) are shipping. The marketing version
     ...
     native file only for the platform(s) shipping _in this release_, or the binary ships the old
  ...
  6. **Submit to the testing track first, never straight to production:**
     ...
     for App Store _review_ is a separate, deliberate action in ASC afterwards. The
     `eas:submit:*:production` variants exist for step 7 and name production in full, so it can't be
     reached by accident.
     - **Never run a bare `eas submit`.** With no `--profile` it silently uses the profile _named_
     ...
     - **Keep [managed publishing](https://support.google.com/googleplay/android-developer/answer/9859654)
       on in Play Console** (Publishing overview → Managed publishing status). It holds _approved_
...
- **Hotfixing the live version while a release branch is mid-cycle**: branch `hotfix/<version>`
  **from the `v<version>` tag of what's actually live** — `git checkout -b hotfix/1.1.5 v1.1.4` —
  ...
  - Then merge it **both** into `main` _and_ into the in-progress `release/<version>`, or it'll be
...
- **React must stay a single version across web + mobile — `19.2.3`, the version Expo bundles.**
  ...
  - **A root react declaration is what breaks this, not what fixes it.** While `next` sat in the
    ...
    satisfy that peer npm fetched the newest match (measured `19.2.8`), hoisted _that_, and pushed
...
- **Verify any dependency-resolution change from a deleted lockfile — then throw that lockfile
  ...
  has been observed introducing duplicate _native_ modules (`expo-constants`, `react-native-screens`,
  ...
  - Check with `npx expo-doctor` in `apps/mobile` — its "no duplicate dependencies" check is the
    ...
    `expo-dev-launcher` entries. Note `npm install` itself re-plants the shims, so clean _after_
...
- **A route file added while Metro is running is "Unmatched Route" until Metro restarts.** Same
  ...
  **"Unmatched Route — Page could not be found"** with `src/app/join/[token].tsx` present, typechecking,
  ...
  the launcher owning `pokerkit://` is the _other_ known trap and makes it easy to blame the wrong
  one.
...
- **`expo-router` does not hoist to the root `node_modules`, and the expo CLI can't find it without
  ...
  - **The path matters more than the package.** npm installs `expo-router` to
    ...
    `node_modules/expo/node_modules/@expo/cli/node_modules/@expo/router-server`. Node walks _up_
    from there, so it sees `node_modules/expo/node_modules` and the root `node_modules`, and never
    `apps/mobile/node_modules`. `NODE_PATH` is appended _after_ that chain, so it fixes the lookup
  ...
  - **Not lockfile rot — don't try to fix it by regenerating.** A full
    ...
    dependencies or its _required_ peers conflicts with root by then, so this is npm's placement
...
- **`expo run:android` plants broken package shims that also break iOS.** **Every affected command
  ...
  root `postinstall` exists and why any manual cleaning must come _after_ installing.
  ...
  That covers shims left over from a _previous_ run; if it still recurs mid-build (a concurrent
  Gradle sync replanting them _during_ the same invocation, or you built via some
  ...
  list and will keep serving a stale one), but deliberately _not_ the default, because the script
  ...
  real, correctly-hoisted copies at root `node_modules/` for _any_ Node-based resolution,
  ...
  - `npx expo-doctor`'s "no duplicate dependencies" check crashes with
    `ENOENT ... apps/mobile/node_modules/expo-constants/package.json` and is reported as a _failed
    check_, which looks like a real duplicate-dependency problem.
...
'Expo'`); patching around _that_ (e.g. adding `pod 'Expo'` to the Podfile by hand) only gets you
...
- **Bumping a native module invalidates every existing dev-client binary — rebuild, don't just
  ...
  the new JS talks to the old native code, which fails at _runtime_ with nothing at build time to
  ...
  platforms before trusting anything you see. `pod install` refusing to run is the _good_ case —
