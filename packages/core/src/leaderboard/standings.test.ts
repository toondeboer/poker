...
describe("computeStandings", () => {
  ...
  it("counts games played for everyone who played, not just who placed", () => {
    // The whole reason playerIds exists: Cy played and placed nowhere, and
    // must still appear with a game to their name.
    const standings = computeStandings(players, [
      game(["a", "b", "c"], [{ playerId: "a", place: 1 }]),
    ]);
    expect(byId(standings, "c").gamesPlayed).toBe(1);
  });

  it("aggregates wins and podiums across games", () => {
    const standings = computeStandings(players, [
      game(
        ["a", "b", "c"],
        [
  ...
  it("counts a fourth-place finish, but not as a podium", () => {
    const four = [...players, createPlayer({ id: "d", name: "Di" })];
    const standings = computeStandings(four, [
      game(["a", "b", "c", "d"], [{ playerId: "d", place: 4 }]),
    ]);
    expect(byId(standings, "d").gamesPlayed).toBe(1);
    expect(byId(standings, "d").podiums).toBe(0);
  });

  it("ranks by wins first", () => {
    const standings = computeStandings(players, [
      game(["a", "b", "c"], [{ playerId: "b", place: 1 }]),
      game(["a", "b", "c"], [{ playerId: "b", place: 1 }]),
      game(["a", "b", "c"], [{ playerId: "a", place: 1 }]),
    ]);
  ...
  it("breaks a wins tie on podiums", () => {
  ...
  it("puts a player who needed fewer games ahead when all else is equal", () => {
    const standings = computeStandings(
      ...
      [
        game(["a", "b"], [{ playerId: "a", place: 1 }]),
        game(["b"], [{ playerId: "b", place: 1 }]),
      ],
    );
  ...
  it("counts a duplicated placing only once", () => {
    ...
    // podiums while gamesPlayed counted 1.
    const standings = computeStandings(players, [
      game(
        ["a", "b"],
        [
