...
const STORE_CONFIG: Record<
  Store,
  { href: string; label: string; gradient: string; path: string }
> = {
  ios: {
    href: APP_STORE_LINK,
...
interface StoreButtonProps {
  store: Store;
  /** Extra layout/shadow classes for the anchor (e.g. `w-full`, `shadow-xl`). */
  className?: string;
  /** Overrides the icon size; defaults to `w-6 h-6 mr-3`. */
...
const StoreButton: React.FC<StoreButtonProps> = ({
  ...
  const { href, label, gradient, path } = STORE_CONFIG[store];
  return (
    <a
      href={href}
      className={`flex items-center px-8 py-4 bg-gradient-to-r ${gradient} text-white font-semibold rounded-xl transition-all transform hover:scale-105 ${className}`}
    >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-8">
              {isMobile ? (
                <div className="w-full max-w-sm space-y-4">
                  <StoreButton
                    store="ios"
                    className="justify-center w-full shadow-lg"
                  />
                  <StoreButton
                    store="android"
                    className="justify-center w-full shadow-lg"
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <StoreButton store="ios" className="shadow-lg" />
                  <StoreButton store="android" className="shadow-lg" />
                </div>
              )}
            </div>
