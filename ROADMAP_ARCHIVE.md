> **Archived 2026-07-21.** This roadmap covered the v1.1 launch + monetization push: iOS/Android
> launch, ads, Pro IAP, ASO, and the first SEO/virality pass. It's closed — see
...
- ⏳ **Web AdSense site review — rejected 2026-07-21** with "Low value content": the site doesn't
  ...
  content pass landed in P3 below (PR #56); resubmitting and building further content depth carries
...
- ✅ **SEO**: enriched web metadata (OG/Twitter/canonical/keywords), added `robots.ts`/`sitemap.ts`,
  gave `/timer` real content (how-to + FAQ, with `FAQPage`/`WebApplication`/`SoftwareApplication`
  JSON-LD) instead of the bare widget — PR #56. This is a necessary on-page/technical baseline, not
  a ranking guarantee — see [ROADMAP.md](./ROADMAP.md) for what's actually needed to rank and drive
  traffic (Search Console, content depth, backlinks).
- ✅ **Table-side virality**: subtle on-screen brand + URL + a share affordance on both the web and
  mobile timer screens — PR #56
...
Superseded by CLAUDE.md's "Things that bite in this monorepo" section — kept here only as a
historical snapshot of what this phase of work ran into:
- iOS builds from source — do NOT re-enable precompiled frameworks.
